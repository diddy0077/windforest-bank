...
const Authentication = ({ openAuth, setOpenAuth }) => {
  ...
  const shakeInput = () => {
    ...
    setTimeout(
      () => otpInputRef.current.classList.remove("animate-shake"),
      500
    );
  };
      ...
      <section className="bg-white relative z-40 shadow-lg rounded-3xl p-6 w-[350px] sm:w-[400px]">
        ...
        {step === 1 && (
          <div className="flex flex-col items-center gap-4">
            ...
            <button
              disabled={isSending}
              onClick={generateOTP}
              className="bg-red-600 text-white p-3 rounded-md w-full transition active:scale-95 cursor-pointer"
            >
        ...
        {step === 2 && (
          <div className="flex flex-col items-center gap-2">
            ...
            <button
              onClick={confirmOTP}
              className="bg-red-600 text-white p-3 rounded-md w-full mt-2 transition active:scale-95 cursor-pointer"
            >
              Confirm OTP
            </button>
            <button
              onClick={resendOTP}
              disabled={timer > 0}
              className="text-sm mt-2 text-blue-600 disabled:text-gray-400 hover:underline cursor-pointer"
            >
              Resend OTP
            </button>
          </div>
