...
export default function TransferSummaryModal({ isOpen, onClose, accountNumber, accountType, transferBeneficiary, amount,setAmount,setFromAccount,setToAccount,setTransferBeneficiary,setIsOpen,addNotification,setNotifications }) {
  ...
  const confirmTransfer = async () => {
    ...
    const res2 = await fetch(`http://localhost:5000/users/?accountNumber=${transferBeneficiary.accountNumber}`)
    const beneficiaryDataArray = await res2.json()
    const beneficiaryData = beneficiaryDataArray[0]
    const newCurrentUserBalance = currentUserData.accountBalance - Number(amount)
    const newBeneficiaryBalance = beneficiaryData.accountBalance + Number(amount)
  ...
  accountBalance: newCurrentUserBalance
    ...
    try {
      ...
      const response2 = await fetch(`http://localhost:5000/users/${beneficiaryData.id}`, {
        method: 'PATCH',
        headers: {
          "Content-Type": "application/json"
        },
...
const updatedSenderNotifications = await addNotification(currentUser.id, senderNotification);
const updatedReceiverNotifications = await addNotification(beneficiaryData.id, receiverNotification);
 ...
 const res3 = await fetch('http://localhost:5000/transactions', {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-6 space-y-4">
          ...
          <div className="flex justify-between border-b border-b-gray-400 pb-2">
            ...
            <span className="font-semibold text-gray-900">{Number(amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</span>
          </div>
          <div className="flex justify-between border-b border-b-gray-400 pb-2">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-bold text-red-600">{calculateTotal(amount)}</span>
          </div>
        </div>
        ...
        <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3">
          ...
          <button onClick={confirmTransfer} className="cursor-pointer active:
      ...
      <TransferSuccessModal isOpen={openSuccessModal} onClose={closeTransfer} amount={amount} recipient={transferBeneficiary?.name} setToAccount={setToAccount} setFromAccount={setFromAccount} setAmount={setAmount} setIsOpen={setIsOpen} />
    </div>
  );
}
