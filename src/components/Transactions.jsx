...
const Transactions = () => {
  ...
  .map(tx => {
    ...
    return {
      ...
      balanceAfter: isDebit ? tx.balanceAfterSender : tx.balanceAfterReceiver
    };
  });
  ...
  return (
    <div className="bg-white shadow-lg rounded-2xl p-4 my-6 container mx-auto md:w-[80%] w-[95%] ">
      <div className='flex items-center gap-3 mb-8'>
        <button onClick={() => setIsSidebarOpen(true)} className="md:hidden">
         ...
         <h2 className="text-xl font-semibold text-gray-800">Recent Transaction History</h2>
      </div>
          ...
          transactions
            ...
            .map((tx, idx) => (
              <tr key={idx} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatDate(tx.date)}</td>
                ...
                <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium text-right ${
                  ...
                  {tx.isDebit ? `-$${Number(tx.amount).toLocaleString()}` : `+$${Number(tx.amount).toLocaleString()}`}
                </td>
  ...
  <div className="md:hidden flex flex-col gap-4">
    {transactions.length > 0 ? (
      transactions
        .sort((a, b) => new Date(b.date) - new Date(a.date))
        .map((tx, idx) => (
          <div key={idx} className="bg-gray-50 rounded-2xl p-4 shadow flex flex-col">
            <div className="flex justify-between mb-1 text-sm text-gray-500">
              <span>{formatDate(tx.date)}</span>
              <span className={tx.isDebit ? 'text-red-600 font-semibold' : 'text-green-600 font-semibold'}>
