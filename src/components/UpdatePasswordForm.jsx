...
const UpdatePasswordForm = ({ setOpenPassword, openPassword,setNotifications }) => {
  ...
  const updatePassword = async (e) => {
    ...
    if (!currentPassword) {
      ...
      toast.error('You must enter your current password to change your password!')
      return
    }
    ...
    const matchedUser = onlineUsers.find((user) => user.userId === currentUser.id)
    if (currentPassword !== matchedUser.password) {
      ...
      const response = await fetch(`http://localhost:5000/onlineAccessUsers/${matchedUser.id}`, {
        method: 'PATCH',
        headers: {
  ...
  type: 'success',
  message: "Your password was changed. If you didn't recognize this change, please contact customer support immediately!",
  date: new Date().toISOString(),
  read: false,
};
      const response2 = await fetch(`http://localhost:5000/users/${currentUser.id}`, {
        method: 'PATCH',
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({notifications: [...currentUser.notifications, newNotification]})
      })
      if (!response2) {
        throw {
  ...
  return (
    <div className={`${openPassword ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} flex justify-center items-center min-h-screen fixed h-screen inset-0 z-40 transition-all duration-300`}>
      ...
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl p-8 mx-4 overflow-y-auto z-50 relative">
        <button onClick={() => setOpenPassword(false)} className='absolute top-4 right-4 cursor-pointer'>
          <svg className='fill-red-600' xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"/></svg>
         </button>
        {/* Header Section */}
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="md:text-4xl text-3xl font-extrabold text-gray-800 text-center mb-1">Update Password</h1>
          <p className="text-center text-gray-500 text-lg">Your security is our priority. Please choose a strong password.</p>
        </div>
        ...
        <form onSubmit={updatePassword} className="space-y-6">
          ...
          <div className='relative'>
            ...
            />
            <div onClick={() => setFirstEye((prev) => !prev)} className='absolute top-9 right-3 cursor-pointer'>
              {firstEye ? <LucideEye className='w-5 h-5'/> : <EyeOff className='w-5 h-5'/>}
            </div>
          </div>
          ...
          <div className='relative'>
            ...
            <input 
              ...
              className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition-all duration-200"
            />
            <div onClick={() => setSecondEye((prev) => !prev)} className='absolute top-9 right-3 cursor-pointer'>
              {secondEye ? <LucideEye className='w-5 h-5'/> : <EyeOff className='w-5 h-5'/>}
            </div>
          </div>
          ...
          <div className='relative'>
            ...
            <input 
              ...
              className="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600 transition-all duration-200"
            />
            <div onClick={() => setThirdEye((prev) => !prev)} className='absolute top-9 right-3 cursor-pointer'>
              {thirdEye ? <LucideEye className='w-5 h-5'/> : <EyeOff className='w-5 h-5'/>}
            </div>
          </div>
