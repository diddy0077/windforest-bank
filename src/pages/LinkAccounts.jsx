...
const BankIcon = ({ className = "w-6 h-6" }) => (
  ...
  >
    <path
      ...
      strokeWidth="2"
      d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-3m0 0a2 2 0 012-2h2a2 2 0 012 2v3"
    />
  </svg>
);
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      const matchedUser = users.find(
        (user) => Number(user.accountNumber) === Number(accountNumber)
      );
      if (matchedUser) {
  ...
  const handleLinkAccount = async () => {
    ...
    if (Number(accountNumber) === Number(currentUser.accountNumber)) {
      ...
      try {
        ...
        toast.success(
          "Two micro-deposits have been successfully sent to the target account. Please ask the account owner to verify the amounts to complete linking."
        );
        setStep(2);
        setAccountNumber("");
      } catch (error) {
        ...
        <div className="w-full max-w-lg bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-200">
          ...
          <div className="p-8 space-y-6">
            <p className="text-gray-600 text-md leading-relaxed">
              Securely link an external account using your account number. We
              will confirm ownership by sending two small micro-deposits (less
              than $1.00) that will arrive immediately.
            </p>
