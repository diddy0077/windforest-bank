...
const ForgotPassword = () => {
  ...
  const handleStep1 = async () => {
    ...
    if (!res.ok) {
      ...
      const matchedOnlineUser = onlineUsers.find((user) => user.userId === matchedUser.id)
  ...
  const handleStep2 = async () => {
    ...
    if (!res.ok) {
      ...
      const users = await res.json()
      const matchedUser = users.find((user) => user.email === userEmail.trim().toLowerCase())
      const res2 = await fetch('http://localhost:5000/onlineAccessUsers')
      const onlineUsers = await res2.json()
      const matchedOnlineUser = onlineUsers.find((user) => user.userId === matchedUser.id)
      if (matchedOnlineUser.securityAnswer.toLowerCase() !== answer?.toLowerCase()) {
  ...
  const handleStep3 = async () => {
    ...
    try {
      ...
      const res3 = await fetch(`http://localhost:5000/users/${user.id}`, {
        ...
        body: JSON.stringify({notifications: [...user.notifications, newNotification]})
      })
      if (!res3.ok) {
        throw {
      ...
      toast.error("Sorry an error occurred, couldn't reset your password, try again later.")
  ...
  const renderFormContent = () => {
    switch (step) {
      case 1:
        return (
          <>
            <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-1">Forgot Password</h1>
            <p className="text-center text-gray-500 text-md mb-8">Enter your email to begin the reset process.</p>
      ...
      case 2:
        return (
          <>
            <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-1">Verify Identity</h1>
            <p className="text-center text-gray-500 text-md mb-8">Please answer your security question to continue.</p>
            <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
            
              
              {/* Mock Security Question 2 */}
              <div>
                <label htmlFor="question" className="block text-sm font-medium text-gray-700">{userQuestion}</label>
                <input
      ...
      case 3:
        return (
          <>
            <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-1">Reset Password</h1>
            <p className="text-center text-gray-500 text-md mb-8">Enter a new, strong password below.</p>
            <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
              <div>
