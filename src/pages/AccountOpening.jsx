...
const AccountOpening = () => {
  ...
  const downloadReceipt = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(brandRed);
    doc.text("Thank you for opening your account with us!", 105, y, {
      align: "center",
    });
  ...
  const validateStep2 = async () => {
    ...
    if (matchedUser) {
      setEmailError(
        "This email is already associated with an account. Please log in or reset your password!"
      );
      toast.error(
        "This email is already associated with an account. Please log in or reset your password!"
      );
              ...
              <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
                {currentStep === 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    />
                    {dobError && (
                      <p className="text-red-600 text-sm">{dobError}</p>
                    )}
                    {fullNameError && (
                      <p className="text-red-600 text-sm">{fullNameError}</p>
                    )}
                    {ssnError && (
                      <p className="text-red-600 text-sm">{ssnError}</p>
                    )}
                  </div>
                )}
                {currentStep === 1 && (
                ...
                {currentStep === 2 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    />{" "}
                    {empStatusError && (
                      <p className="text-red-600 text-sm">{empStatusError}</p>
                    )}
                    {annualIcomError && (
                      <p className="text-red-600 text-sm">{annualIcomError}</p>
                    )}
                    {empNameError && (
                      <p className="text-red-600 text-sm">{empNameError}</p>
                    )}
                  </div>
                )}{" "}
                {currentStep === 3 && (
                ...
                {currentStep === 4 && (
                  <div className="space-y-4">
                    ...
                    <p className="text-gray-600">
                      ...
                      submitting. This section would display a summary of all
