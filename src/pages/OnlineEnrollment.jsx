...
const OnlineEnrollment = () => {
 ...
 useEffect(() => {
  ...
  const steps = [
    ...
    { title: 'Account Creation', fields: ['username', 'password', 'securityQuestion', 'securityAnswer'] },
...
const handleSubmit = async (e) => {
  ...
  const matchedUser = users.find(
    (user) =>
      user.fullName.toLowerCase().trim() === formData.fullName.toLowerCase().trim() &&
      user.ssn === formData.ssn &&
      user.dob === formData.dob
  );
  ...
  termsAccepted: formData.termsAccepted,
    ...
    if (!createRes.ok) throw new Error("Failed to create online access account.");
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 0:
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-red-600">Personal Information</h2>
            <input
              type="text"
              name="fullName"
              ...
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.fullName ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            />
            {errors.fullName && <p className="text-sm text-red-500 -mt-4">{errors.fullName}</p>}
            <input
              type="text"
              name="ssn"
              ...
              onChange={handleChange}
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.ssn ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            />
            {errors.ssn && <p className="text-sm text-red-500 -mt-4">{errors.ssn}</p>}
            <input
              type="date"
              name="dob"
              value={formData.dob}
              onChange={handleChange}
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.dob ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            />
            {errors.dob && <p className="text-sm text-red-500 -mt-4">{errors.dob}</p>}
          </div>
        );
      ...
      case 1:
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-red-600">Account Creation</h2>
            <input
              type="text"
              name="username"
              ...
              onChange={handleChange}
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.username ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            />
            {errors.username && <p className="text-sm text-red-500 -mt-4">{errors.username}</p>}
            <input
              type="password"
              name="password"
            ...
            <select
              ...
              onChange={handleChange}
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.securityQuestion ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            >
            ...
            {errors.securityQuestion && <p className="text-sm text-red-500 -mt-4">{errors.securityQuestion}</p>}
            <input
              type="text"
              name="securityAnswer"
              ...
              onChange={handleChange}
              className={`w-full p-4 border rounded-xl focus:outline-none focus:ring-2 ${
                errors.securityAnswer ? 'border-red-500' : 'border-gray-300 focus:ring-red-500'
              }`}
            />
            {errors.securityAnswer && <p className="text-sm text-red-500 -mt-4">{errors.securityAnswer}</p>}
          </div>
        );
      case 2:
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-red-600">Terms and Conditions</h2>
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 h-64 overflow-y-scroll">
              <p className="text-sm text-gray-700 leading-relaxed">
                By continuing, you agree to the following terms and conditions. These terms govern your use of our online banking services. We may update these terms from time to time, and your continued use constitutes acceptance of the new terms. Your privacy is important to us. We collect, use, and share information as described in our Privacy Policy.
                <br /><br />
                Security is a shared responsibility. You are responsible for keeping your login credentials confidential. Notify us immediately of any unauthorized use of your account. We reserve the right to suspend or terminate your account at any time for any reason.
              </p>
            </div>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-red-600">Review Your Details</h2>
            ...
            <div className="p-6 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="text-gray-700">
                  <span className="font-semibold text-gray-900">Full Name:</span> {formData.fullName}
                </div>
                <div className="text-gray-700">
                  <span className="font-semibold text-gray-900">Date of Birth:</span> {formData.dob}
                </div>
                
                <div className="text-gray-700">
                  <span className="font-semibold text-gray-900">Email:</span> {formData.email}
                </div>
                <div className="text-gray-700">
                  <span className="font-semibold text-gray-900">Phone:</span> {formData.phone}
                </div>
                <div className="text-gray-700">
                  <span className="font-semibold text-gray-900">Username:</span> {formData.username}
                </div>
              </div>
              <div className="mt-4">
                <span className="font-semibold text-gray-900">Address:</span> {formData.address}
              </div>
              <div className="mt-4">
                <span className="font-semibold text-gray-900">SSN:</span> {formData.ssn}
              </div>
      ...
      <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-2xl shadow-lg max-w-sm text-center transform scale-100 transition-transform duration-300">
              ...
              <p className="text-gray-600 mb-6">
                Thank you for enrolling. You can now log in to your new online banking account.
              </p>
              <button
              ...
              <div key={index} className="flex flex-col items-center">
                ...
                <div
                  className={`mt-2 text-xs sm:text-sm font-medium whitespace-nowrap text-center transition-colors duration-500 ${
                    index === currentStep ? 'text-red-700 font-bold' : 'text-gray-500'
                  }`}
                >
                  {step.title}
                </div>
