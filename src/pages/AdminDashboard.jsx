...
const AdminDashboard = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const fullUserArray = users.map((user) => {
          const matchedOnline = onlineAccessUsers.find(
            (u) => u.userId === user.id
          );
  ...
  const reverseTransaction = async (user, tx) => {
    try {
      ...
      await patchUser(receiver.id, {
        accountBalance: newReceiverBalance,
        notifications: [...receiver.notifications, receiverNotification],
      });
      await patchUser(sender.id, {
        accountBalance: newSenderBalance,
        notifications: [...sender.notifications, senderNotification],
      });
      ...
      setSelectedUser((prev) => ({
        ...prev,
        isTransferEnabled: !prev.isTransferEnabled,
      }));
      ...
      toast.success(
        `Transaction of $${Number(tx.amount).toLocaleString(undefined, {
    ...
    <div className="p-4 sm:p-8 bg-gray-50 min-h-screen font-sans">
      <header className="mb-8 flex justify-between items-center border-b pb-4">
        ...
        <div className="text-right">
          ...
          <p className="font-mono text-xs text-red-600 bg-red-100 p-1 rounded-md">
            {userId}
          </p>
          <button
            onClick={logout}
      ...
      <div
        id="admin-alert"
        className="hidden fixed top-4 right-4 bg-red-600 text-white p-3 rounded-lg shadow-xl z-50 transition-opacity duration-300"
      ></div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-red-50">
            <tr>
              ...
              <th className="px-6 py-3 text-right text-xs font-bold text-red-800 uppercase tracking-wider">
                Account Balance
              </th>
              <th className="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">
                Transfer Status
              </th>
              <th className="px-6 py-3 text-center text-xs font-bold text-red-800 uppercase tracking-wider">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map((user) => (
              <tr
                key={user.userId}
                className="hover:bg-red-50/20 transition duration-150 ease-in-out"
              >
                ...
                <td
                  ...
                  style={{
                    color: user.accountBalance < 0 ? "#b91c1c" : "#10b981",
                  }}
                >
                  {/* Using user.accountBalance */}
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="p-5 rounded-xl border-4 border-red-100 shadow-lg">
              <p className="text-md font-semibold text-gray-600">
                Current Account Balance ({user.accountType})
              </p>
              <p
            ...
            >
              ...
              <p
                className={`font-semibold ${
                  user.isTransferEnabled ? "text-green-700" : "text-red-700"
                } mb-3`}
              >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <DetailItem
                icon={CreditCard}
                title="Debit Card"
                value={user.debitCard ? "Active" : "Inactive"}
              />
              <DetailItem
                icon={Laptop}
                title="Online Banking"
                value={user.onlineBanking ? "Active" : "Inactive"}
              />
              <DetailItem
                icon={Home}
                title="Employer"
            ...
            <h3 className="text-xl font-semibold text-gray-800 border-b pb-2 mt-8">
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200 text-sm">
                        ...
                        <div className="mt-3">
                          {!tx.reversed ? (
                            ...
                            >
                              <RefreshCw className="w-3 h-3 mr-1" /> Reverse
                              Transaction
                            </BrandButton>
                          ) : (
        ...
        <footer className="p-4 border-t flex justify-end">
          <button
            onClick={onClose}
            className="cursor-pointer text-gray-600 hover:text-gray-800 font-semibold p-2"
          >
            Close Panel
          </button>
        </footer>
